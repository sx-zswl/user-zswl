 ...
 *
 * @author: Roy
 * @update: 3/31/2013  二次封装原有函数，增加项目中需要的功能，简化使用
 * @update: 4/10/2013  优化overflowTip算法，无需指定截取长度，改为自动判断
 */

(function($) {
/**
 * 对过长字符截取显示，同时浮动层显示完整内容
 * Usage:	$("#tableId").overflowTip();
 * 页面元素: 需增加属性float="overflow"
 */
$.fn.overflowTip = function() {
	var $floats = $(this).find("[float='overflow']");
	//在外循环，提高性能
	$floats.each(function() {
		$item = $(this);

		//处理表单元素
		var hasFormObj = $("select,a,img,input,button,textarea,ul,li,div,table", $item).size() > 0 ? true : false;
		if(hasFormObj) {
			return true;
		}

		var itemWidth = $item.width();
		var charLen = itemWidth/6 - 3;
		...
		var textWidth = msg.getCharLength() * 6;

		if(itemWidth > 0 && textWidth > itemWidth) {
			$item.attr("title", msg).tooltip({
				track: true
			});
			msg = msg.getCharOfFixedLength(charLen) + "...";
			$item.text(msg);
		}
	});
}
...
/**
 * 使用title属性浮动显示完整内容
 * Usage:	$("#tableId").overflowTipLight();
 * 页面元素: 需增加属性float="overflow"
 */
$.fn.overflowTip4Table = function() {
	$(this).each(function() {
		var $table = $(this);
		var $floatsTr = $("tr:first", $("tbody", $table)).find("td[float='overflow']");
		var $floats = $table.find("td[float='overflow']");

		var cellWidths = new Array();
		var charNums = new Array();
		$floatsTr.each(function() {
			var $td = $(this);
			var cellWidth = $td.width();
			var charNum = cellWidth/6 - 2;
			cellWidths[$td.index()] = cellWidth;
			charNums[$td.index()] = charNum;
		});
		...
		$floats.each(function() {
			...
			var cellWidth = cellWidths[$item.index()];
			var charNum = charNums[$item.index()];
...
/**
 * 金额查看时提示人民币大写
 * Usage:	$("#tableId").rmbTip();
 * 页面元素: 需增加属性float="rmb"
 */
$.fn.rmbTip = function() {
	var jObj = $(this).find("[float='rmb']");
	//在外循环，提高性能
	jObj.each(function() {
		//浮动tip
		var msg = $(this).text().toRmbCnUpper();
		$(this).attr("title", msg).tooltip({
				track: true
		});
	});
}
...
/**
 * 输入金额的input框浮动人民币大写提示
 * Usage:	$("#tip").editRmbTip();
 * 页面元素: 需增加属性float="editRmb"
 */
$.fn.editRmbTip = function() {
	var jObj = $(this).find(":text[float='editRmb']");
	//在外循环，提高性能
	jObj.each(function() {
		//设置动态tip
		$(this).on("keyup", function() {
			...
			$(this).attr("tip", value.toRmbCnUpper());
		});
		//浮动tip
		$(this).powerFloat({
				offsets: {
		...
		//针对只读的input框
		$(this).attr("tip", $(this).val().toRmbCnUpper());
		//浮动tip
		$(this).powerFloat({
				offsets: {
					x: 0,
...
var o = {
	...
	position: function() {
		...
		//Modified by Roy on 3/31/2013 for 解决tip太短时小三角位置问题
...
$.fn.stayBottom = function() {
	...
	$divs.each(function() {
		...
		$(window).off("scroll.stayBottom");
		$(window).on("scroll.stayBottom", function() {
