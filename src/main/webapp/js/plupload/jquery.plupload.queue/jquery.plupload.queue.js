	...
	$.fn.pluploadQueue = function(settings) {
		if (settings) {
			this.each(function() {
				...
				//Added by Roy on 8/15/2013 to fix issues
				...
				//Modified by Roy on 8/14/2013 to handle file information from reponse
				uploader.bind('FileUploaded', function(up, file, response) {
					...
					if(status.code != 0) {
						...
						file.hint = status.message;
					}
		
					handleStatus(file);

					if(settings.callback && $.isFunction(settings.callback)) {
						settings.callback(item, status);
					}
				});
