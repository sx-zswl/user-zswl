	...
	accordion: function(options, data) {
		...
		return this.each(function() {
			...
			// INIT with optional options
				...
				$.data(this, "dwz-accordion", new $.ht.accordion(this, options));
		});
	},
	/**
...
$.ht.accordion = function(container, options) {
	
	// setup configuration
	this.options = options = $.extend({}, $.ht.accordion.defaults, options);
...
function fillSpace(key){
	...
	var height = parent.height() - (($(".accordion_header", obj).size()) * ($(".accordion_header:first-child", obj).outerHeight())) -2;

	var os = parent.children().not(obj);
	$.each(os, function(i){
		height -= $(os[i]).outerHeight();
	});
	$(".accordion_content",obj).height(height);
}
...
function toggle(toShow, toHide, data, clickedActive, down) {
	...
	if ( options.animated ) {
		if ( !options.alwaysOpen && clickedActive ) {
			$.ht.accordion.animations[options.animated]({
				toShow: jQuery([]),
				toHide: toHide,
				complete: complete,
				down: down,
